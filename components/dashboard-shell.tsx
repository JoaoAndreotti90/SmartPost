...
import { LayoutDashboard, History, CreditCard, Sparkles, Menu, X } from "lucide-react"
...
export function DashboardShell({ user }: DashboardShellProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden relative">
      ...
      <aside className={`
        bg-white border-r border-slate-100 p-6 
        md:flex md:flex-col md:w-72 h-full 
        ${isSidebarOpen ? 'fixed inset-0 z-50 flex flex-col w-full' : 'hidden'}
      `}>
        ...
        <nav className="space-y-1 flex-1">
          <Link href="/" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-lg bg-blue-50 text-blue-700 font-medium transition-all group">
             <LayoutDashboard size={18} className="text-blue-600" />
             Gerar Post
          </Link>
          
          <Link href="/history" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-lg text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all font-medium group">
             <History size={18} className="text-slate-400 group-hover:text-slate-600 transition-colors" />
             Histórico
          </Link>

          <Link href="/pricing" onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-lg text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all font-medium group">
